...
if dir<>'' then labelededit8.Text:=cambiar_path(dir+'\');
end;
...
if dir<>'' then labelededit9.Text:=cambiar_path(dir+'\');
end;
...
if dir<>'' then labelededit10.Text:=cambiar_path(dir+'\');
end;
...
begin
  ...
  labelededit7.Text:=cambiar_path(main_config.dir_base+'extras\maps\');
  labelededit8.Text:=cambiar_path(main_config.dir_base+'extras\walk\');
  labelededit9.Text:=cambiar_path(main_config.dir_base+'zip_games\');
  labelededit10.Text:=cambiar_path(main_config.dir_base+'extras\imgs\');
  labelededit11.Text:=cambiar_path(main_config.dir_base+'extras\mt32');
...
begin
  ...
  {$else}
  Opendialog1.Filter:='Dosbox file (*.*)|*.*';
  {$endif}
...
begin
  ...
  Opendialog1.Filter:='EXE file (*.exe)|*.exe';
  {$else}
  Opendialog1.Filter:='Dosbox-X file (*.*)|*.*';
  {$endif}
...
begin
  ...
  Opendialog1.Filter:='EXE file (*.exe)|*.exe';
  {$else}
  Opendialog1.Filter:='ScummVM file (*.*)|*.*';
  {$endif}
...
if dir<>'' then labelededit6.Text:=cambiar_path(dir+'\');
end;
...
if dir<>'' then labelededit7.Text:=cambiar_path(dir+'\');
end;
