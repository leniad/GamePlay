...
type
  tipo_idioma=record
    principal:array[0..25] of string;
    grabar:array[0..33] of string;
    error:array[0..7] of string;
...
var
  idioma_ind:byte;
  list_error:array[0..7] of string;
...
const
  ...
  'Error en la sección de guías. Revisa que:'+FULL_RET+'  - Si es una carpeta debe terminar en ''\'' y debe existir dentro de [DIR_BASE]extras\walk'+FULL_RET+'  - Si es un fichero debe existir en [DIR_BASE]extras\walk'+FULL_RET+'  - Si son varios ficheros deben estar separados por el simbolo ''$'' y deben existir todos en [DIR_BASE]extras\walk','Error: uno de los parámetros es incorrecto, no puedo localizar el CDROM del juego','Error: uno de los parámetros es incorrecto, no puedo localizar el setup del juego'+FULL_RET+'Revisa el nombre del fichero o el directorio del juego!','Error: uno de los parámetros es incorrecto, no puedo localizar el ejecutable del juego'+FULL_RET+'Revisa el nombre del fichero o el directorio del juego!','Precaución: El juego se va a borrar!'+FULL_RET+'¿Deseas continuar?',
  'El juego seleccionado no se puede ejecutar, por favor revisa la configuración');
  ...
  //Ingles
  (principal:('Game Filters','Languaje','Type','Spanish','English','German','French','Italian','Graphic Adventure','Simulator','Sports','Racing','Main Options','Engine','Full Screen','Sound','Show info/help message','Advanced Options','Add Game','Edit/Delete Game','Game Search','Game Images','Game Info','Company:','Year:','About');
  grabar:('Add Game','Game','Full Name','Publication Year','Company','Image Name','Executable','Folder','Executable File','Parameters','Secondary Disk Image','Execute BEFORE','Execute AFTER','Setup file','CPU Cycles','Computer Type','RAM Memory','Enable GUS','Compatible with ScummVM','Keyboard Map File','Help/Info Message','DOSBox Extra Parameters','Extra Info','Manual(s)','Map(s)','Walk Through','Languaje','Type','ACCEPT','CANCEL','DELETE','ZIP File','ZIP file name','ZIP compressed file');
  ...
  'Error in the Walk Through section. Check that:'+FULL_RET+'  - If it is a folder, it must end with ''\'' and exist within [DIR_BASE]extras\walk'+FULL_RET+'  - If it is a file, it must exist in [DIR_BASE]extras\walk'+FULL_RET+'  - If there are multiple files, they must be separated by the ''$'' symbol and all must exist in [DIR_BASE]extras\walk','Error: One of the parameters is incorrect, I cannot locate the game''s CD-ROM','Error: One of the parameters is incorrect, I cannot locate the game''s setup.'+FULL_RET+'Check the file name or the game directory!','Error: One of the parameters is incorrect, I cannot locate the game''s executable'+FULL_RET+'Check the file name or the game directory!','Warning: The game is going to be deleted!'+FULL_RET+'Do you want to continue?',
  'The selected game cannot be run, please check the settings');
  ...
  //Aleman
  (principal:('Spielfilter','Sprache','Typ','Spanisch','Englisch','Deutsch','Französisch','Italienisch','Grafik-Adventure','Simulator','Sport','Rennen','Hauptoptionen','Engine','Vollbild','Sound','Info-/Hilfemeldung anzeigen','Erweiterte Optionen','Hinzufügen Spiel','Bearbeiten/Löschen Spiel','Spielsuche','Spielbilder','Spielinfo','Firma:','Jahr:','Über');
  grabar:('Spiel hinzufügen','Spiel','Vollständiger Name','Erscheinungsjahr','Firma','Image-Name','Ausführbare Datei','Ordner','Name der ausführbaren Datei','Parameter','Sekundäres Disk-Image','Ausführen VORHER','Ausführen NACHHER','Setup-Datei','CPU-Zyklen','Computertyp','RAM-Speicher','GUS aktivieren','Kompatibel mit ScummVM','Tastaturbelegungsdatei','Hilfe-/Info-Meldung','Zusätzliche DOSBox-Parameter','Zusätzliche Informationen','Handbuch(e)','Karte(n)','Anleitung','Sprache','Typ','AKZEPTIEREN','ABBRECHEN','LÖSCHEN','ZIP-Datei','ZIP-Dateiname','ZIP-komprimierte Datei');
  ...
  'Fehler im Walkthrough-Bereich. Überprüfe folgendes:'+FULL_RET+'  - Wenn es ein Ordner ist, muss er mit ''\'' enden und innerhalb von [DIR_BASE]extras\walk existieren.'+FULL_RET+'  - Wenn es eine Datei ist, muss sie in [DIR_BASE]extras\walk existieren.'+FULL_RET+'  - Wenn es mehrere Dateien sind, müssen sie durch das ''$''-Symbol getrennt sein und alle in [DIR_BASE]extras\walk existieren','Fehler: Einer der Parameter ist incorrect, ich kann das CD-ROM des Spiels nicht finden','Fehler: Einer der Parameter ist incorrect, ich kann das Setup des Spiels nicht finden.'+FULL_RET+'Überprüfe den Dateinamen oder das Spielverzeichnis!','Fehler: Einer der Parameter ist incorrect, ich kann die ausführbare Datei des Spiels nicht finden.'+FULL_RET+'Überprüfe den Dateinamen oder das Spielverzeichnis!','Warnung: Das Spiel wird gelöscht!'+FULL_RET+'Möchtest du fortfahren?',
  'Das ausgewählte Spiel kann nicht gestartet werden, bitte überprüfe die Einstellungen');
  ...
  //Frances
  (principal:('Filtres du jeu','Langue','Type','Espagnol','Anglais','Allemand','Français','Italien','Aventure graphique','Simulateur','Sports','Courses','Options générales','Moteur','Plein écran','Son','Afficher le message info/aide','Options avancées','Ajouter Jeu','Modifier/Supprimer Jeu','Rechercher un jeu','Images du jeu','Informations sur le jeu','Entreprise :','Année :','À propos');
  grabar:('Ajouter un jeu','Jeu','Nom complet','Année de publication','Entreprise','Nom de l''image','Exécutable','Répertoire','Nom de l''exécutable','Paramètres','Image du deuxième disque','Exécuter AVANT','Exécuter APRÈS','Programme d''installation','Cycles CPU','Type d''ordinateur','Mémoire RAM','Activer GUS','Compatible avec ScummVM','Fichier de mappage du clavier','Message d''information/aide','Paramètres supplémentaires DOSBox','Informations supplémentaires','Manuel(s)','Carte(s)','Guide(s)','Langue','Type','ACCEPTER','ANNULER','SUPPRIMER','Fichier ZIP','Nom du fichier ZIP','Fichier compressé ZIP');
  ...
  'Erreur dans la section des guides. Vérifiez que :'+FULL_RET+'  - Si c''est un dossier, il doit se terminer par ''\'' et exister dans [DIR_BASE]extras\walk'+FULL_RET+'  - Si c''est un fichier, il doit exister dans [DIR_BASE]extras\walk'+FULL_RET+'  - Si ce sont plusieurs fichiers, ils doivent être séparés par le symbole ''$'' et tous doivent exister dans [DIR_BASE]extras\walk.','Erreur : Un des paramètres est incorrect, je ne peux pas localiser le CD-ROM du jeu','Erreur: Un des paramètres est incorrect, je ne peux pas localiser le setup du jeu.'+FULL_RET+'Vérifiez le nom du fichier ou le répertoire du jeu!','Erreur : Un des paramètres est incorrect, je ne peux pas localiser l''exécutable du jeu.'+FULL_RET+'Vérifiez le nom du fichier ou le répertoire du jeu!','Avertissement: Le jeu va être supprimé!'+FULL_RET+'Souhaitez-vous continuer?',
  'Le jeu sélectionné ne peut pas être lancé, veuillez vérifier la configuration');
  ...
  'Errore nella sezione delle guide. Verifica che:'+FULL_RET+'  - Se è una cartella, deve terminare con ''\'' ed esistere all''interno di [DIR_BASE]extras\walk'+FULL_RET+'  - Se è un file, deve esistere in [DIR_BASE]extras\walk'+FULL_RET+'  - Se sono più file, devono essere separati dal simbolo ''$'' e devono esistere tutti in [DIR_BASE]extras\walk','Errore: uno dei parametri è errato, non riesco a localizzare il CD-ROM del gioco','Errore: uno dei parametri è errato, non riesco a localizzare il setup del gioco'+FULL_RET+'Controlla il nome del file o la directory del gioco!','Errore: uno dei parametri è errato, non riesco a localizzare l''eseguibile del gioco'+FULL_RET+'Controlla il nome del file o la directory del gioco!','Attenzione: il gioco verrà eliminato!'+FULL_RET+'Vuoi continuare?',
  'Il gioco selezionato non può essere avviato, per favore controlla le impostazioni');
...
begin
  ...
  form1.LabeledEdit1.EditLabel.Caption:=idioma_fijo[idioma_ind].principal[20];
  form1.groupbox2.caption:=idioma_fijo[idioma_ind].principal[21];
  ...
  form1.bitbtn5.caption:=idioma_fijo[idioma_ind].principal[25];
  {$ifndef fpc}
  form1.Button3.hint:=idioma_fijo[idioma_ind].avanzado[1];
  {$else}
...
begin
  ...
  form2.combobox4.Items.Add('General');
  ...
  form2.CheckBox3.Caption:=idioma_fijo[idioma_ind].grabar[33];
end;
...
begin
  ...
  form4.RadioGroup1.Caption:=idioma_fijo[idioma_ind].principal[1];
